...
**Descente dans les blocs.** Un template range volontiers ses bases dans des colonnes, une bascule ou un encart : `GET /v1/blocks/{id}/children` ne rendant que le premier niveau, la découverte traverse récursivement les conteneurs de mise en page, avec une profondeur bornée à 5. Deux arrêts nets : les pages enfants ne sont pas suivies — ce sont d'autres pages, les suivre reviendrait à explorer le workspace — et les bases non plus, leurs enfants étant des lignes et non des blocs. Les blocs visités sont mémorisés, un `synced_block` pouvant renvoyer vers un ancêtre.
